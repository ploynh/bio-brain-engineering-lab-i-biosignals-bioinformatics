...
filtered_blink_left_alpha = filter(C, D, rawLeftEEG);
filtered_blink_right_alpha = filter(C, D, rawRightEEG);
% filtered beta
Betafreq = [14 30];
[C,D] = butter(6, Betafreq/(SampleF/2),'bandpass')
filtered_blink_left_beta = filter(C, D, rawLeftEEG);
filtered_blink_right_beta = filter(C, D, rawRightEEG);
%filtered theta
Thetafreq = [4 7];
[C,D] = butter(6, Thetafreq/(SampleF/2),'bandpass')
filtered_blink_left_theta = filter(C, D, rawLeftEEG);
filtered_blink_right_theta = filter(C, D, rawRightEEG);

% Plot 5 raw + filtered signal (blink_left_right) [Alpha]
% Subplot 1 raw_blink_left_alpha
figure
subplot(2,2,1);
plot(Time, rawLeftAlpha,'Color',[1 0 0]);
xlabel('Time (s)');
ylabel('Amplitude (\muV)');
title('Raw signal of blinking');
hold on;
% Subplot 2 filtered_blink_left_alpha
subplot(2,2,3);
plot(Time, filtered_blink_left_alpha,'Color',[0 0 0]);
xlabel('Time (s)');
ylabel('Amplitude (\muV)');
title('Filtered signal of blinking');
% Subplot 3 raw_blink_right_alpha
subplot(2,2,2);
...
title('Raw signal of blinking');
hold on;
% Subplot 4 filtered_blink_right_alpha
subplot(2,2,4);
plot(Time, filtered_blink_right_alpha,'Color',[0 0 0]);
xlabel('Time (s)');
ylabel('Amplitude (\muV)');
title('Filtered signal of blinking');
sgtitle('Alpha band of left (left) and right (right) Hemispheres');

% Plot 6 raw + filtered signal (blink_left_right) [beta]
% Subplot 1 raw_blink_left_beta
figure
subplot(2,2,1);
...
title('Raw signal of blinking');
hold on;
% Subplot 2 filtered_blink_left_beta
subplot(2,2,3);
plot(Time, filtered_blink_left_beta,'Color',[0 0 0]);
xlabel('Time (s)');
ylabel('Amplitude (\muV)');
title('Filtered signal of blinking');
% Subplot 3 raw_blink_right_beta
subplot(2,2,2);
...
title('Raw signal of blinking');
hold on;
% Subplot 4 filtered_blink_right_beta
subplot(2,2,4);
plot(Time, filtered_blink_right_beta,'Color',[0 0 0]);
xlabel('Time (s)');
ylabel('Amplitude (\muV)');
title('Filtered signal of blinking');
sgtitle('Beta band of left (left) and right (right) Hemispheres');

% Plot 7 filtered signal (blink_left_right) [Theta]
% Subplot 1 filtered signal (blink_left) [Theta]
figure
subplot(2,1,1);
plot(Time, filtered_blink_left_theta,'Color',[0 0 0]);
xlabel('Time (s)');
ylabel('Amplitude (\muV)');
title('Left hemisphere');
hold on;
% Subplot 2 filtered signal (blink_right) [Theta]
subplot(2,1,2);
plot(Time, filtered_blink_right_theta,'Color',[0 0 0]);
xlabel('Time (s)');
ylabel('Amplitude (\muV)');
title('Right hemisphere');
sgtitle('Theta band of blinking');
